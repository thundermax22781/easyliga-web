  ...
  const loadPlayer = async () => {
    ...
    try {
      ...
      if (found) {
        ...
        setDob(found.birth_date ? new Date(found.birth_date) : new Date());
        setRole(found.role);
      ...
      <SafeAreaView style={[styles.container, dynamicStyles.container]} edges={['top']}>
        <ScrollView showsVerticalScrollIndicator={false}>
          ...
          <View style={styles.profileSection}>
            ...
            {player.name && <Text style={[styles.profileName, dynamicStyles.subText]}>{player.name}</Text>}
          ...
          <View style={[styles.detailsCard, dynamicStyles.card]}>
            ...
            <View style={styles.detailRow}><Text style={[styles.detailLabel, dynamicStyles.subText]}>Nome Reale</Text><Text style={[styles.detailValue, dynamicStyles.text]}>{player.name || '-'}</Text></View>
            <View style={[styles.detailDivider, dynamicStyles.divider]} />
            <View style={styles.detailRow}><Text style={[styles.detailLabel, dynamicStyles.subText]}>Data di Nascita</Text><Text style={[styles.detailValue, dynamicStyles.text]}>{player.birth_date ? new Date(player.birth_date).toLocaleDateString('it-IT') : '-'}</Text></View>
          </View>
        </ScrollView>
      </SafeAreaView>
    );
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : 'height'} style={{ flex: 1 }}>
        <TouchableWithoutFeedback onPress={Keyboard.dismiss}>
          <ScrollView showsVerticalScrollIndicator={false} contentContainerStyle={styles.editContent}>
            ...
            <View style={styles.inputGroup}>
              <Text style={[styles.label, dynamicStyles.text]}>Nome Reale</Text>
              <TextInput style={[styles.input, dynamicStyles.input]} value={name} onChangeText={setName} placeholder="Nome e Cognome" placeholderTextColor={isDarkMode ? "#8E8E93" : "#C7C7CC"} />
            </View>
