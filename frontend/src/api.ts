...
export const checkPremiumStatus = async (): Promise<boolean> => {
  try {
    // 1. Controllo istantaneo cache locale (Orologio Svizzero)
    const cached = await AsyncStorage.getItem('is_premium_user');
    if (cached === 'true') return true;

    // 2. Controllo Cloud (se non Premium in locale)
...
export const redeemCode = async (code: string): Promise<void> => {
  ...
  const WHITELIST = ["PRO-EASYLIGA2026", "W@LCOME-PRO-MEMBER2026"];

  const { data: userData } = await supabase.auth.getUser();
  const user = userData?.user;
  if (!user) throw new Error("Connessione mancante. Attendi il login.");

  // MASTER BYPASS: Se il codice è nella whitelist, sblocchiamo subito
  if (WHITELIST.includes(cleanCode)) {
    await AsyncStorage.setItem('is_premium_user', 'true');
    // Registriamo comunque sul Cloud in background per coerenza
    supabase.from('premium_users').upsert([{ user_id: user.id }]).then(() => {});
    return;
  }

  // LOGICA ROBUSTA PER FUTURI UTENTI PAGANTI
  // 1. Verifica codice (Case-Insensitive con ilike)
  ...
  // 2. Segna codice come usato (Aggiornamento atomico)
  ...
  if (updateError) throw new Error("Errore durante l'attivazione. Riprova.");

  // 3. Registra Utente Premium
  const { error: premiumError } = await supabase
    .from('premium_users')
    .upsert([{ user_id: user.id }]);

  if (premiumError) throw new Error("Errore salvataggio profilo. Contatta il supporto.");

  // 4. Salvataggio locale definitivo
  await AsyncStorage.setItem('is_premium_user', 'true');
};
