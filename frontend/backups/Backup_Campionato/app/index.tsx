    ...
    try {
      if (editGroup) {
        ...
        await createGroupExtended(groupName.trim(), storageType, {
          ...
          tournament_teams_count: teamsCount
        });
      }
      resetForm();
      loadGroups();
              ...
              >
                ...
                <View style={styles.groupInfo}>
                  <View style={{ flexDirection: 'row', alignItems: 'center', gap: 6, paddingRight: 4 }}>
                    <Text style={[styles.groupName, dynamicStyles.text, { flex: 1 }]} numberOfLines={1}>{item.name || 'Senza Nome'}</Text>
                  </View>
                  <View style={[styles.groupMeta, { flexWrap: 'wrap', paddingRight: 8 }]}>
                    <Text style={[styles.groupCount, dynamicStyles.subText]} numberOfLines={1}>{item.player_count || 0} giocatori</Text>
                    {item.group_type === 'tournament' && (
                      <>
                        <View style={[styles.metaDot]} />
                        <Text style={[styles.groupCount, dynamicStyles.subText]} numberOfLines={1}>{item.tournament_teams_count || 4} squadre</Text>
                      </>
                    )}
                  </View>
                </View>

                <View style={[styles.groupActions, { flexDirection: 'column', alignItems: 'flex-end', gap: 6 }]}>
                  <View style={{ backgroundColor: item.group_type === 'tournament' ? '#5856D6' : '#007AFF', paddingHorizontal: 6, paddingVertical: 2, borderRadius: 6 }}>
                ...
                {/* Status indicator absolute at bottom-right */}
                <View style={{ position: 'absolute', bottom: 10, right: 15, flexDirection: 'row', alignItems: 'center' }}>
                    {item.storage_type === 'cloud' ? (
                      <>
                        <Text style={{ fontSize: 9, fontWeight: '900', color: (item.role === 'owner') ? '#FFB800' : (item.role === 'admin' ? '#007AFF' : '#FF3B30'), textTransform: 'uppercase', letterSpacing: 0.5 }}>
                          {item.role === 'owner' ? 'Proprietario' : item.role === 'admin' ? 'Amministratore' : 'SOLO LETTURA'}
                        </Text>
                        <View style={[styles.metaDot, { width: 2, height: 2, opacity: 0.5, marginHorizontal: 4 }]} />
                        <Text style={{color: '#34C759', fontSize: 9, fontWeight: '800', textTransform: 'uppercase'}}>
                          Condiviso
                        </Text>
                      </>
                    ) : (
                      <Text style={{color: isDarkMode ? '#AEAEB2' : '#8E8E93', fontSize: 9, fontWeight: '800', textTransform: 'uppercase'}}>
                        Locale
                      </Text>
                    )}
                </View>
