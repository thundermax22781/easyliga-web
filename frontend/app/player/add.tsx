...
export default function AddPlayerScreen() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      Alert.alert('Errore Cloud', e.message || 'Impossibile salvare il giocatore su Supabase. Controlla la connessione o i permessi.');
    } finally {
      setSaving(false);
    }
  };
        ...
        <TouchableWithoutFeedback onPress={() => setShowStrengthModal(false)}>
          <View style={styles.modalOverlay}>
            <View style={[styles.modalContent, dynamicStyles.modalContent]}>
              ...
              <ScrollView><View style={styles.strengthGrid}>
                {STRENGTH_VALUES.map((v) => (
                  <TouchableOpacity key={v} style={[styles.strengthOption, dynamicStyles.option, strength === v && styles.strengthOptionSelected]} onPress={() => { setStrength(v); setShowStrengthModal(false); }}>
                    <Text style={[styles.strengthOptionText, dynamicStyles.text, strength === v && {color: '#FFF'}]}>{v}</Text>
                  </TouchableOpacity>
                ))}
              </View></ScrollView>
            </View>
